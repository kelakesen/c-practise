...
int Monster_attack(void *self,int damage){
        ...
        printf("You attack the %s with %d HP\n",monster->_(description),damage);
...
int Player_attack(void *self,int damage){
	...
	printf("You are attacked for %d \n",damage);

	mapplayer->player->HP -= damage;

	if(mapplayer->player->HP > 0){
		printf("You are alive with %d HP \n",mapplayer->player->HP);
		...
		printf("You will be teleported back to hall,try again \n");

		mapplayer->_(init)(mapplayer);
...
int process_input(Map *game)
{
    ...
    switch(ch) {
	    ...
	    if(game->location->bad_guy)game->player->_(attack)(game,damage1);
