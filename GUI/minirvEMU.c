...
void Draw_SW(uint32_t addr,uint32_t *RGB);
// 1. 定义全部CPU运行状态
typedef enum {
    STATE_IF,    // 取指
    STATE_ID,    // 译码拆分指令
...
int main(int argc,char *argv[]){
    ...
    uint32_t R[32] = {0};
    uint32_t *RGB = malloc(sizeof(uint32_t));

    ioe_init();

    FILE *fp = fopen("/home/kelakesen/test/logisim-bin/vga.bin","rb");
    ...
    uint32_t inst=0, opcode=0, rd=0, rs1=0, rs2=0, addr=0,funct3=0,funct7=0,word_addr=0,byte_select=0;
    uint32_t imm=0;
    ...
    while(1)
    {
		...
		//printf("%"PRIx32"\n",funct3_opcode);
		    ...
		    case 0x123://SW
			   ...
			   M[addr] = R[rs2];
			   *RGB = R[rs2];
			   if(0x08010000>addr && addr>=0x08000000)Draw_SW(addr,RGB);
			   pc += 4; 
			break; 
	  	    case 0x23://SB
...
void Draw_SW(uint32_t addr,uint32_t *RGB){
	if(0x08010000>addr && addr>=0x08000000){
		int x = (addr - 0x08000000)%256;
		int y = (addr - 0x08000000)/256;
		io_write(AM_GPU_FBDRAW,x,y,RGB,1,1,true);
	}
}
