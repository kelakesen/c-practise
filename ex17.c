...
struct Database {struct Address rows[MAX_ROWS];};
...
void Database_lode(struct Connection* conn){
	int rc = fread(conn->db,sizeof(struct Database),1,conn->file);
	if(rc!=1)die("Lode error");
}	
...
void Database_set(struct Connection* conn,int id,char* name,char* email){
	...
	if(!res)die("name copy error");

	res = strncpy(addr->email,email,MAX_DATA-1);
	addr->email[MAX_DATA-1]='\0';
	if(!res)die("email copy error");
}	
...
void Database_delete(struct Connection* conn,int id){
	struct Address addr = {.set=0,.id=id};
...
int main(int argc,char* argv[]){
    ...
    if(argc > 3 && action == 'f') name = argv[3];
